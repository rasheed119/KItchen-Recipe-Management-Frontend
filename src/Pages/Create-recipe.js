...
import { Box, Button, CircularProgress, Container, Input, Typography } from '@mui/material';
...
function Createrecipe() {
  ...
  const handlesubmit = async(e) => {
    ...
    if(UserID()){
      try {
        await axios.post("https://kitchen-recipe-backend-nu.vercel.app/recipes",recipe);
  ...
  return (
    ...
    sx={{
      ...
      <Box component="form" onSubmit={(e)=>handlesubmit(e)}  sx={{ mt:3 , display : 'flex' ,flexDirection : 'column' }} >
        ...
        <Box sx={{ mt:2,mb:2 }} >
          ...
          <Box sx={{ display: 'flex',flexDirection: 'column' }} >
            ...
            <Box key={index} >
              ...
              />
              <ClearIcon onClick={()=>handleingredientdelete(index)} sx={{ cursor : 'pointer' }} />
            </Box>
            )) }

          <Button variant="contained" type='button' onClick={addingredients} >Add Ingredients</Button>
          ...
          <Box sx={{ display: 'flex',flexDirection: 'column' }} >
            ...
            { recipe.instructions.map((event,index)=>(
              <Box key={index} >
                ...
                />
                <ClearIcon onClick={()=>handleinstructionsdelete(index)} sx={{ cursor : 'pointer' }} />
              </Box>
            )) }

            <Button variant='contained' type='button' onClick={addinstructions} >Add Instructions</Button>
          </Box>
        </Box>
        ...
        { show ?
        <Button variant='contained' type='submit' sx={{ mt:2,mb:2 }} >Create Recipe</Button>
        :
